function [missingMets,directions] = diagnoseMissingMet(model,ess_rxn)

% Takes in a reaction that is essential for growth, turns off that
% reaction, and creates a source/sink for each metabolite.  Turns one of
% those source/sinks off at a time to diagnose which of the metabolites in
% the reaction are essential
%
% INPUT
% model: a COBRA Toolbox model structure
% ess_rxn: an essential reaction in the supplied model
%
% OUTPUT
% missingMets: the list of metabolites in rxn that require this reaction as
% a source or a sink
...
model = changeRxnBounds(model,ess_rxn,0,'b');

% Step 2: Find all the metabolites in the reaction
mets = findMetsFromRxns(model,ess_rxn);
