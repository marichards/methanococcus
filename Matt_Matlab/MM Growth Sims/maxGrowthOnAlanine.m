function [solution,gibbs_flux,model] = maxGrowthOnAlanine(model,substrate_rxns,concentrations,print_flag)

% Simulate M. maripaludis growth on CO2 and H2 media, with alanine as the
...
% Make sure that alanine is on, not nitrogen or ammonia
model = switchToAlanine(model);
...
if print_flag
    ...
    [~,ala_idx] = intersect(model.rxns,'EX_cpd00035[e0]');
